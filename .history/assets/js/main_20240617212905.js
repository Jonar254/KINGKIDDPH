...
function playVideo(videoId, autoplay = false) {
    ...
    topPlayer.innerHTML = '<iframe width="560" height="315" src="' + videoUrl + '" frameborder="0" allowfullscreen></iframe>';

    // Smooth scroll to the top of the top player section with adjusted offset
    window.scrollTo({
        top: topPlayer.getBoundingClientRect().top + window.scrollY - 100, // Adjusted offset to avoid navigation bar
        behavior: "smooth"
    });
...
function updatePreviousNextButtons(videoId) {
    ...
    for (var i = 0; i < videoContainers.length; i++) {
        var videoURL = videoContainers[i].querySelector("iframe").src;
    ...
    if (currentIndex > 0) {
        var previousVideoId = videoContainers[currentIndex - 1].querySelector("iframe").src.split("/").pop();
    ...
    if (currentIndex < videoContainers.length - 1) {
        var nextVideoId = videoContainers[currentIndex + 1].querySelector("iframe").src.split("/").pop();
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    videoContainers.forEach(function(container) {
        container.addEventListener("click", function() {
            ...
            var videoURL = this.querySelector("iframe").src;
...
function scrollSlider() {
 ...
 scrollSlider() here to disable continuous scrolling
